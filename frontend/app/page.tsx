...
function ProviderModal({health,onClose,onChanged}:{health:Health|null;onClose:()=>void;onChanged:()=>Promise<void>}){
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-black/65 p-4 backdrop-blur-sm" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}>
    <div className="panel w-full max-w-lg p-5 shadow-2xl">
      ...
      <label className="mb-3 block text-xs text-slate-400">Model<input className="input mt-1" value={model} onChange={e=>setModel(e.target.value)} placeholder="model identifier"/></label>
